// adaptation of pbpb_angantyr_5tev.cc to zBosons.
...
int main(int argc, char* argv[]) {
  ...
  if(argc > 1){ //includes number in file name
	outFileString.append("/store/user/pierrarj/collisions_pbpb/allParts/ba");
	outFileString.append(argv[1]);
	outFileString.append("/jb");
	outFileString.append(argv[2]);
        outFileString.append("/tree_allParts_ba");
	outFileString.append(argv[1]);
	outFileString.append("jb");
	outFileString.append(argv[2]);
  ...
  pythia.init();
    ...
    TTree *allTree = new TTree("allTree", "Tree with all particle output from PYTHIA8");
    allTree->Branch("nMultiplicityTree", &nMultiplicityTree, "nMultiplicityTree/I");
    allTree->Branch("bTree", &bTree, "bTree/D");
    allTree->Branch("pidTree", &pidTree, "pidTree[nMultiplicityTree]/I");
  ...
  for ( int iEvent = 0; iEvent < nEvents; ++iEvent ) {
    ...
    cout<<"Running event number: "<<iEvent<<endl;
    ...
    for (int i = 0; i < pythia.event.size(); ++i){
      ...
      if (p.isFinal() && abs(p.eta()) < 2.4 && p.pT() > 0.1){
