#!/bin/sh
nbatch=90 #batch number
njobs=100 #number of jobs in batch

ban=ba$nbatch
mkdir $ban
mkdir /store/user/pierrarj/collisions_pbpb/$ban #create batch directory for storage
...
jbi=jb$i
mkdir $ban/$jbi
mkdir /store/user/pierrarj/collisions_pbpb/$ban/$jbi #create job directory for storage
