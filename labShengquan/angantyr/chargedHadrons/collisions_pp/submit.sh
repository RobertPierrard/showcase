#!/bin/sh
nbatch=103 #batch number
njobs=100 #number of jobs in batch

ban=ba$nbatch
mkdir $ban
mkdir /store/user/pierrarj/collisions_pp/$ban #create batch directory for storage
...
jbi=jb$i
mkdir $ban/$jbi
mkdir /store/user/pierrarj/collisions_pp/$ban/$jbi #create job directory for storage

cp pp_angantyr_5tev $ban/$jbi
