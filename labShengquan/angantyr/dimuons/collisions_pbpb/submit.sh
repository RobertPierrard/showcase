...
njobs=1 #number of jobs in batch

ban=ba$nbatch
mkdir $ban
mkdir /store/user/pierrarj/dimuons/collisions_pbpb/$ban #create batch directory for storage
...
echo $i
jbi=jb$i
mkdir $ban/$jbi #create job directory for storing code and console output
mkdir /store/user/pierrarj/dimuons/collisions_pbpb/$ban/$jbi #create job directory for storage

cp pbpb_angantyr_5tev_dimuons $ban/$jbi
