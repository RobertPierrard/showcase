...
void analysisPtSpectraPbpb(string inRootFile){
  ...
  for(int i=0; i<nCentralityBins; i++){
    ...
    for(int j=0; j< ptSpectra[i]->GetNbinsX(); j++){
      ...
      ptSpectraWeighted[i]->SetBinContent(j+1, ptSpectraWeighted[i]->GetBinContent(j+1)*1.0/dEtaBin/dPtBins[j]/(2.0*TMath::Pi()*ptSpectra[i]->GetBinCenter(j+1)));
      ptSpectraWeighted[i]->SetBinError(j+1, ptSpectraWeighted[i]->GetBinError(j+1)*1.0/dEtaBin/dPtBins[j]/(2.0*TMath::Pi()*ptSpectra[i]->GetBinCenter(j+1)));
    }
  ...
  // create output file
  TFile *outFile = new TFile("normalized_output_hist.root","recreate");
  outFile->cd();
