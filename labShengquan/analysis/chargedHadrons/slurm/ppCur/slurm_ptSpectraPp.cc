...
void ptSpectraPp(string binBoundaryFilename, string rootFilelistFilename, string centralityParam){
  ...
  centralityParamDist = new TH1D("hist_centralityParamDist","", 200, 0, 20);
  ptSpectra = new TH1D("hist_ptSpectra","", nPtBins, ptBins);
  ptSpectraWeighted = new TH1D("hist_ptSpectraWeighted","", nPtBins, ptBins);

  // -----------------------------------------------------------------------------------------
  // input root files from filelist
     cout << "Inputting root files from filelist..." << endl;
     ...
     while ( flist >> ffname ){
       t1.Add(ffname);
       if((fileCounter%20)==0) cout << "Opened " << fileCounter << " files" << endl;
       fileCounter++;
     }
     cout << endl << endl;
     
     cout << "Setting branch addressess." << endl;
 ...
 //    t1.SetBranchAddress("nPartTree",&nPartTree);
     ...
     cout << "Retrieving total number of entries." << endl;
     nEvt=t1.GetEntries();
  ...
  //   - there are two times where we need to branch depending on the centrality paramter used.
     ...
     //nEvt = 10000;
     cout << "nEvt = " << nEvt << "Looping through events..." << endl << endl;
...
// ----------------------------------------------------------------------------------------
// Main function for calling the above functions.
// ----------------------------------------------------------------------------------------
int main(int argc, char *argv[]){
  string binBoundaryFilename(argv[1]);
  string rootFilelistFilename(argv[2]);
  if(argc <= 3){
    ptSpectraPp(binBoundaryFilename, rootFilelistFilename);
  }else{  
    string centralityParam(argv[3]);
    ptSpectraPp(binBoundaryFilename, rootFilelistFilename, centralityParam);
  }
  return 0;
}
