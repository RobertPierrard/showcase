...
void analysisPtSpectraPp(string inRootFile){
  ...
  centralityParamDist = (TH1D*)inFile->Get("hist_centralityParamDistPp");
  ptSpectra = (TH1D*)inFile->Get("hist_ptSpectraPp");
  ptSpectraWeighted = (TH1D*)inFile->Get("hist_ptSpectraWeightedPp");

  centralityParamDist->SetName("hist_normalizedCentralityParamDistPp");
  ptSpectra->SetName("hist_normalizedPtSpectraPp");
  ptSpectraWeighted->SetName("hist_normalizedPtSpectraWeightedPp");
  ...
  TFile *outFile = new TFile("normalized_output_hist_pp2Bil.root","recreate");
  outFile->cd();
